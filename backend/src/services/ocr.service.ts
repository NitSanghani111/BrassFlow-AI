import { OCRDocument, OCRStatus, OCRDocumentType } from '@prisma/client';
import { prisma } from '../config/prisma';
import { logger } from '../utils/logger';

export class OCRService {
  async processDocument(documentId: string): Promise<OCRDocument> {
    logger.info(`Starting asynchronous AI OCR process for document: ${documentId}`);
    ...
    try {
      ...
      if (doc.documentType === OCRDocumentType.PURCHASE_BILL) {
        extractedData = {
          vendor: {
      ...
      const updatedDoc = await prisma.oCRDocument.update({
        where: { id: documentId },
        data: {
      ...
      logger.info(`OCR processing finished successfully for: ${documentId}`);
      return updatedDoc;
    } catch (error: any) {
      logger.error(`OCR processing failed for document ${documentId}: ${error.message}`);
