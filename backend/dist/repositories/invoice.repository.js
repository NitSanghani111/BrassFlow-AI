...
class InvoiceRepository {
    ...
    async create(data, userId) {
        return prisma_1.prisma.$transaction(async (tx) => {
            ...
            for (const item of data.items) {
                ...
                await tx.stockTransaction.create({
                    data: {
                        productId: item.productId,
                        quantity: item.quantity,
                        type: client_1.StockTransactionType.OUT,
                        reason: client_1.StockReason.INVOICE_BILLING,
            ...
            const pdfPath = await (0, pdfGenerator_1.generateInvoicePDF)(pdfData);
            // Update Invoice with pdfUrl (local static path uploads/invoices/...)
            const relativePdfUrl = `/uploads/invoices/${path_1.default.basename(pdfPath)}`;
