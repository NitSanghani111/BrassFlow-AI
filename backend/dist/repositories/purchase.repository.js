...
class PurchaseRepository {
    ...
    async findById(id) {
        return prisma_1.prisma.purchase.findUnique({
            where: { id },
            include: {
                supplier: true,
                items: {
            ...
            for (const item of data.items) {
                ...
                await tx.stockTransaction.create({
                    data: {
                        productId: item.productId,
                        quantity: item.quantity,
                        type: client_1.StockTransactionType.IN,
                        reason: client_1.StockReason.PURCHASE_RECEIPT,
    ...
    async updateStatus(id, status, userId) {
        const purchase = await prisma_1.prisma.purchase.update({
            where: { id },
            data: { status },
        });
